...
const ViewDreamTeam = () => {
  ...
  // Fetch a team by username programmatically (used by explore list)
  const fetchTeamByUsername = async (u) => {
    setLoading(true);
    setError('');
    setTeam(null);
    try {
      const res = await apiClient.get(`dreamteam/username/${u}`);
      setTeam(res.data);
      setUsername(u);
      if (!res.data.players || res.data.players.length === 0) {
        setError(`${u} hasn't created a dream team yet.`);
      }
    } catch (err) {
      if (err.response && err.response.status === 404) {
        setError(`User "${u}" not found. Please check the username and try again.`);
      } else {
        setError('Something went wrong while searching. Please try again.');
      }
    }
  ...
  useEffect(() => {
    // Load recent dream teams for exploration
    let mounted = true;
    (async () => {
      try {
        const res = await apiClient.get('dreamteam');
        if (mounted) setExploreTeams(res.data.teams || []);
      } catch (err) {
        // ignore failures, explore list is optional
      }
    })();
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-white py-16 px-4">
      <div className="max-w-4xl mx-auto p-8 bg-slate-900 rounded-2xl shadow-xl">
      <div className="text-center mb-8">
        <h2 className="text-4xl font-extrabold text-indigo-200 mb-3 tracking-tight drop-shadow">Explore Dream Teams</h2>
        <p className="text-slate-300 text-lg">Discover and analyze other players' dream team selections</p>
      </div>
      
        <div className="bg-blue-900/20 rounded-xl shadow-lg p-6 mb-8">
        {exploreTeams && exploreTeams.length > 0 && (
          <div className="mb-6">
            <h3 className="text-lg font-semibold mb-3">Recent Dream Teams</h3>
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
              {exploreTeams.map(t => (
                <div key={t.id} className="flex items-center justify-between bg-blue-900/10 p-3 rounded text-white">
                  <div>
                    <div className="font-semibold">{t.username || 'Unknown'}</div>
                    <div className="text-sm text-gray-500">{t.players?.length || 0} players{t.name ? ` • ${t.name}` : ''}</div>
                  </div>
                  <div>
                    <button onClick={() => fetchTeamByUsername(t.username)} className="bg-blue-600 text-white px-3 py-1 rounded">Open</button>
                  </div>
                </div>
              ))}
            </div>
      ...
      {error && (
        <div className="bg-red-900/20 border border-red-700/30 rounded-xl p-6 mb-8 text-red-200">
      ...
      {team && team.players && team.players.length > 0 && (
        <div className="space-y-8">
          <div className="bg-blue-900/10 rounded-xl shadow-lg p-6 border-l-4 border-green-500 text-white">
          ...
          <div className="bg-blue-900/10 rounded-xl shadow-lg overflow-hidden">
            <div className="bg-gradient-to-r from-green-600 to-green-700 text-white p-4">
              ...
              <div className="p-4 space-y-3">{team.players.slice(0, 5).map((player, index) => (
                <div key={player._id} className="flex items-center justify-between bg-blue-900/10 rounded-lg p-4 hover:bg-blue-900/20 transition group text-white">
                  ...
                  <div className="flex gap-6 text-center">
                    <div className="bg-blue-800/20 rounded-lg px-3 py-2 shadow-sm">
                      <div className="font-bold text-green-200">{player.stats?.points ?? player.stats?.pointsPerGame ?? '-'}</div>
                      <div className="text-xs text-slate-200">PPG</div>
                    </div>
                    <div className="bg-blue-800/20 rounded-lg px-3 py-2 shadow-sm">
                      <div className="font-bold text-green-200">{player.stats?.assists ?? player.stats?.assistsPerGame ?? '-'}</div>
                      <div className="text-xs text-slate-200">APG</div>
                    </div>
                    <div className="bg-blue-800/20 rounded-lg px-3 py-2 shadow-sm">
                      <div className="font-bold text-green-200">{player.stats?.rebounds ?? player.stats?.reboundsPerGame ?? '-'}</div>
                      <div className="text-xs text-slate-200">RPG</div>
                    </div>
                  </div>
                </div>
              ))}
              ...
              <div className="p-4 space-y-3">
                {team.players.slice(5, 10).map((player, index) => (
                  <div key={player._id} className="flex items-center justify-between bg-blue-900/10 rounded-lg p-4 hover:bg-blue-900/20 transition group text-white">
                    ...
                    <div className="flex gap-6 text-center">
                      <div className="bg-blue-800/20 rounded-lg px-3 py-2 shadow-sm">
                        <div className="font-bold text-yellow-200">{player.stats?.points ?? player.stats?.pointsPerGame ?? '-'}</div>
                        <div className="text-xs text-slate-200">PPG</div>
                      </div>
                      <div className="bg-blue-800/20 rounded-lg px-3 py-2 shadow-sm">
                        <div className="font-bold text-yellow-200">{player.stats?.assists ?? player.stats?.assistsPerGame ?? '-'}</div>
                        <div className="text-xs text-slate-200">APG</div>
                      </div>
                      <div className="bg-blue-800/20 rounded-lg px-3 py-2 shadow-sm">
                        <div className="font-bold text-yellow-200">{player.stats?.rebounds ?? player.stats?.reboundsPerGame ?? '-'}</div>
                        <div className="text-xs text-slate-200">RPG</div>
                      </div>
                    </div>
                  </div>
      ...
      {team && (!team.players || team.players.length === 0) && (
        <div className="bg-blue-900/20 border border-blue-800/30 rounded-xl p-8 text-center text-white">
          <div className="flex flex-col items-center gap-4">
            <div className="bg-blue-800/30 rounded-full p-4">
              <svg className="w-12 h-12 text-blue-500" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M19 11H5m14 0a2 2 0 012 2v6a2 2 0 01-2 2H5a2 2 0 01-2-2v-6a2 2 0 012-2m14 0V9a2 2 0 00-2-2M5 11V9a2 2 0 012-2m0 0V5a2 2 0 012-2h6a2 2 0 012 2v2M7 7h10" />
              </svg>
            </div>
            <div>
              <h3 className="text-white font-semibold text-xl mb-2">No Dream Team Yet</h3>
              <p className="text-slate-200 text-lg">This player hasn't created their dream team yet.</p>
              <p className="text-slate-300 text-sm mt-2">Check back later or try searching for another player!</p>
            </div>
          </div>
        </div>
      )}
